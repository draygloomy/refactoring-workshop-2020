...
bool Controller::isSegmentAtPosition(Segment seg) const
{
    return m_segments.end() !=  std::find_if(m_segments.cbegin(), m_segments.cend(),
        [seg](auto const& segment){ return segment.x == seg.x and segment.y == seg.y; });
}

bool Controller::isPositionOutsideMap(Segment seg) const
{
    return x < 0 or y < 0 or x >= m_mapDimension.first or y >= m_mapDimension.second;
}

void Controller::sendPlaceNewFood(Segment seg)
{
    m_foodPosition = std::make_pair(seg.x, seg.y);
...
{
    Segment seg{newHead.x, newHead.y};

    if (isSegmentAtPosition(seg) or isPositionOutsideMap(seg)) {
        m_scorePort.send(std::make_unique<EventT<LooseInd>>());
    } else {
        addHeadSegment(seg);
        removeTailSegmentIfNotScored(seg);
    }
}
...
void Controller::updateFoodPosition(Segment seg, std::function<void()> clearPolicy)
{
    if (isSegmentAtPosition(seg) || isPositionOutsideMap(seg)) {
        m_foodPort.send(std::make_unique<EventT<FoodReq>>());
        return;
    }
...
void Controller::handleFoodInd(std::unique_ptr<Event> e)
{
    ...
    updateFoodPosition(Segment{receivedFood.x, receivedFood.y}, std::bind(&Controller::sendClearOldFood, this));
}
...
void Controller::handleFoodResp(std::unique_ptr<Event> e)
{
    ...
    updateFoodPosition(Segment{requestedFood.x, requestedFood.y}, []{});
}
